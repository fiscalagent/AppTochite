...
- ✅ Sharpening: Z-1 (приёмка) + Z-2 (заточка) — `sharpening.*` видимый текст, дропдаун типа камня
  из словаря, даты/деньги по локали. Диктовочные тосты + `fieldLabel` → Фаза 3 (батч 9).
  Placeholder «выберите тип абразива» закрыт через `t.reference.selectTypePlaceholder` (батч 9).
