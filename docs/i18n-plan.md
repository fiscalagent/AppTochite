...
- ⚠️ **status (StatusPill)** — конфликт формы подписи: плашка показывает `принят`/`готов`, а
  CSV-экспорт `Принят`/`Готово`. Нужно решение, какую форму считать канонической подписью (или
  держать две). Отложено до согласования.
- ⚠️ **Дропдауны Reference (тип/СОЖ)** — текст `<option>` захардкожен с другой капитализацией
  («Гальваника» vs «гальваника» в списке). Унификация = видимое изменение → решить в текстовом
  проходе. Safety-инвариант уже соблюдён (пишут ключи).
...
- ⏳ common / навигация (BottomNav, Layout, общие кнопки)
- ⏳ Clients (C-1/C-2/C-3)
- ⏳ Sharpening (Z-1 приёмка / Z-2 заточка)
- ⏳ History (H-1)
...
- ⚠️ **Валюта** — символ ₽ захардкожен (напр. «Цена, ₽» в Z-1). Для en нужна валюта — **продуктовое
  решение** (какая?). Технически решается `fmtMoney`, но дефолт надо выбрать.
