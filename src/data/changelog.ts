...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: '2026-07-24',
    changes: [
      'увеличение фото профиля клиента по клику — на карточке клиента, в списке клиентов и на цифровой визитке',
    ],
  },
  {
    version: '2.6.3',
    date: '2026-07-18',
    changes: [
      'аналитика могла уронить всё приложение белым экраном на старых Android WebView — crypto.randomUUID() вызывался без фолбэка на верхнем уровне модуля',
      'быстрое удаление двух фото «до» подряд на экране заточки могло отменить одно из удалений',
      'вкладка «Справочники» в нижней навигации не подсвечивалась на подвкладках Стали и Ножи',
      'голосовой ввод чисел с «тысяча»/«hundred» считал сложением, а не умножением — «две тысячи» превращались в 1002 вместо 2000',
      'дневной гейт папочного и облачного авто-бэкапа терял правки, внесённые после первой проверки дня — теперь гейт только по сигнатуре данных, как уже было исправлено для APK',
      'импорт ножей из файла считал дубликатом любой нож с тем же брендом, даже с другой сталью — терял легитимные варианты вроде разных сталей одной модели',
      'кнопка «Ещё» в ленте истории могла пропасть раньше, чем реально подгружены все заточки, из-за мягко удалённых записей в корзине',
      'критический баннер бэкапа не исчезал при переходе на экран бэкапа внутри приложения без перезагрузки страницы',
      'напоминание о бэкапе, закрытое на низком уровне, не показывалось повторно при эскалации до warn/critical в той же сессии',
      'пагинация в карточке клиента теряла заточки, если в окне страницы попадали мягко удалённые записи из корзины',
      'реимпорт CSV камней ломался на точке с запятой внутри названия — экспорт всегда кавычит поля, а импорт их не распознавал',
    ],
  },
  {
    version: '2.6.2',
  ...
  },
  {
    ...
    date: '2026-07-13',
    changes: [
      'аппаратная кнопка «назад» в APK закрывала приложение вместо навигации назад по экранам',
    ],
  },
  {
    version: '2.5.1',
  ...
  },
  {
    ...
    date: '2026-07-11',
    changes: [
      'авто-бэкап после приёмки/готово не подвешивает интерфейс и не парсит JSON дважды',
      'история заточек и карточка клиента грузят только текущую страницу, а не всю таблицу с фото',
      'приёмка заточки не читает справочник ножей дважды при показе подсказок',
    ],
  },
  {
    version: '2.4.8',
    date: '2026-07-11',
    changes: [
      'список клиентов и подсказки ножа на приёмке не тянут все фото заточек',
    ],
  },
  {
    version: '2.4.7',
  ...
  },
  {
    ...
    date: '2026-07-06',
    changes: [
      'длинные списки на экранах свернуты для компактности — в «О программе» раздел «Что нового» показывает три последние версии, а на экране бэкапа список копий для восстановления — три самые свежие; остальное раскрывается кнопкой «Показать более ранние»',
    ],
  },
  {
    version: '2.2.0',
    date: '2026-07-06',
    changes: [
      'кнопка «Сообщить об ошибке» на экране «О программе» — опишите проблему, оставьте ник в Telegram для ответа, и сообщение уйдёт разработчику вместе с технической информацией (версия, тип установки, устройство, состояние хранилища); работает и без сети — отправится при появлении соединения',
    ],
  },
  {
    version: '2.1.2',
    date: '2026-07-06',
    changes: [
      'карточка защиты данных теперь зеленеет от одной подключённой папки в APK-версии (раньше не учитывала её и требовала ещё и облако), а после подключения Яндекс.Диска первый бэкап уходит в облако сразу — без ожидания суточного авто-бэкапа',
      'приложение больше не предлагает обновление, пока новый APK ещё собирается — раньше в первые минуты после выхода релиза кнопка скачивания вела на ошибку 404; теперь плашка и кнопки появляются только когда файл реально прикреплён к релизу, и качают его по точной ссылке',
    ],
  },
  {
    version: '2.1.1',
  ...
  },
  {
    ...
    date: '2026-07-06',
    changes: [
      'обновление приложения в APK-версии — при выходе новой версии на экране «О программе» появляется кнопка «Скачать (APK)»: файл качается в браузере, открываете его из загрузок — приложение обновляется, все данные сохраняются',
    ],
  },
  {
    version: '2.0.5',
  ...
  },
  {
    ...
    date: '2026-07-06',
    changes: [
      'бэкап в папку — выбор папки больше не пропадает впустую на телефонах, выгружающих приложение во время системного диалога (Samsung, Xiaomi и др.): выбранная папка теперь запоминается и подключается при возврате в приложение, а при ошибке записи показывается её причина',
    ],
  },
  {
    version: '2.0.3',
  ...
  },
  {
    ...
    date: '2026-07-06',
    changes: [
      'бэкап в папку на устройстве в приложении наконец работает — теперь вы сами выбираете папку в системном диалоге, и копия сохраняется в неё (на новых Android прямая запись в Документы была запрещена, отсюда ошибка «нет доступа к папке»)',
      'если приложение работает во временном хранилище браузера (данные может стереть Chrome) — теперь предупреждаем об этом и подсказываем установить приложение на экран телефона; при отказе записи в базу показываем ошибку, а не теряем данные молча',
    ],
  },
  {
    version: '2.0.1',
  ...
  },
  {
    ...
    date: '2026-06-25',
    changes: [
      'AppTochite теперь можно установить как приложение для Android — со страницы загрузки, без магазина',
      'Приложение и браузерная версия хранят данные раздельно: чтобы перенести записи, сделайте резервную копию в браузере и восстановите её в приложении',
    ],
  },
  {
    version: '1.109.1',
    date: '2026-06-25',
    changes: [
      'при «Повторить заточку» в новую заточку переносится микроподвод — его угол и сама отметка',
      'камень, заведённый прямо в заточке, теперь сохраняется и в справочник — с распознанной из названия гритностью; в подсказке кнопка «Уточнить тип и гритность» для дозаполнения типа',
    ],
  },
  {
    version: '1.109.0',
  ...
  },
  {
    ...
    date: '2026-06-24',
    changes: [
      'Восстановление из папки на устройстве починено — копия снова открывается (раньше могло ошибочно писать «Файл в папке не найден или повреждён»)',
    ],
  },
  {
    version: '1.108.0',
    date: '2026-06-24',
    changes: [
      'Автобэкап в папку — не чаще раза в сутки и только при изменении данных (раньше копия переписывалась при каждом открытии приложения)',
    ],
  },
  {
    version: '1.107.0',
    date: '2026-06-24',
    changes: [
      'Экран «Настройки» переработан — бэкап стал понятнее и надёжнее',
      'Резервные копии выстроены по надёжности: в браузере → папка на устройстве → облако',
      'Восстановление — единый список доступных копий с датой и размером: сразу видно, какая свежее и полнее',
      'Поделиться, выгрузка JSON и Excel/CSV перенесены в раздел «Дополнительно»',
      'Исправлена запись бэкапа в папку на устройстве',
    ],
  },
  {
    version: '1.106.1',
    date: '2026-06-23',
    changes: [
      'Восстановление из облака упрощено: список снимков теперь справочный — чтобы развернуть бэкап, скачайте файл в приложении Яндекс.Диск и откройте его через «Восстановить из файла»',
      '«Заменить всё» при восстановлении из файла теперь спрашивает подтверждение',
    ],
  },
  {
    version: '1.106.0',
  ...
  },
  {
    ...
    date: '2026-06-23',
    changes: [
      'Скачивание бэкапа из облака починено — файл сохраняется системным загрузчиком',
      'Объединить данные из облака: скачайте снапшот, затем выберите его в «Восстановить из файла» → «Объединить»',
    ],
  },
  {
    version: '1.105.2',
  ...
  },
  {
    ...
    date: '2026-06-23',
    changes: [
      'подсказка для iPhone — как добавить приложение на экран «Домой» через Safari',
      'предложение установить приложение — после первой заточки и баннером для возвращающихся',
      'угол заточки и микроподвод регулируются ползунком с шагом 0,1° и шкалой 10–45°',
      'добавлен угол микроподвода (МП) на экране заточки',
      'кнопка «Установить приложение» на экране «О программе» и в онбординге',
      'кнопка «Поделиться приложением» на главном экране клиентов — ссылка на лендинг для сарафана',
      'мини-игра «Верный угол» — тренажёр глазомера на угол заточки',
    ],
  },
  {
    version: '1.105.0',
  ...
  },
  {
    ...
    date: '2026-06-21',
    changes: [
      'Кнопка «Установить приложение» — на приветственном экране и в разделе «О программе»',
    ],
  },
  {
    version: '1.100.0',
    date: '2026-06-21',
    changes: [
      'Кнопка «Поделиться приложением» на главном экране — отправьте друзьям и клиентам ссылку на приложение',
    ],
  },
  {
    version: '1.99.0',
    date: '2026-06-21',
    changes: [
      'Мини-игра «Верный угол» — тренажёр глазомера на угол заточки (в разделе «Игры»)',
    ],
  },
  {
    version: '1.98.1',
