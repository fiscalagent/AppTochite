...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: '2026-06-24',
    changes: [
      'восстановление из папки на устройстве — «Файл в папке не найден или повреждён» при живом файле; чтение теперь повышает разрешение под тапом (раньше только запрашивало статус и срывалось в prompt)',
      'автобэкап в папку — как у облака: не чаще раза в сутки и только при изменении данных (раньше писал при каждом открытии приложения)',
      'новый экран бэкапа — лесенка надёжности (в браузере → папка → облако), единый список копий для восстановления, ручные выгрузки в «Дополнительно»',
    ],
  },
  {
    version: '1.108.0',
