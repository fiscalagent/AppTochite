...
* фильтрация камней в справочнике по типу абразива и СОЖ ([](https://github.com/fiscalagent/AppTochite/commit/fdcde4db70dfdaa0d41ac66a6ef9fef572e4156c))
...
* длинные списки на экранах свернуты для компактности — в «О программе» раздел «Что нового» показывает три последние версии, а на экране бэкапа список копий для восстановления — три самые свежие; остальное раскрывается кнопкой «Показать более ранние» ([](https://github.com/fiscalagent/AppTochite/commit/f477c997c167e2b109c24835e4aecbc9dce0f2af))

# [2.2.0](https://github.com/fiscalagent/AppTochite/compare/v2.1.2...v2.2.0) (2026-07-06)


### feat

* кнопка «Сообщить об ошибке» на экране «О программе» — опишите проблему, оставьте ник в Telegram для ответа, и сообщение уйдёт разработчику вместе с технической информацией (версия, тип установки, устройство, состояние хранилища); работает и без сети — отправится при появлении соединения ([](https://github.com/fiscalagent/AppTochite/commit/bb315292b3f71c581c62d69a7c1d31eda2463d83))

## [2.1.2](https://github.com/fiscalagent/AppTochite/compare/v2.1.1...v2.1.2) (2026-07-06)


### fix

* карточка защиты данных теперь зеленеет от одной подключённой папки в APK-версии (раньше не учитывала её и требовала ещё и облако), а после подключения Яндекс.Диска первый бэкап уходит в облако сразу — без ожидания суточного авто-бэкапа ([](https://github.com/fiscalagent/AppTochite/commit/f232c1df67262de658132b7ab83b5f7bc48c7902))
* приложение больше не предлагает обновление, пока новый APK ещё собирается — раньше в первые минуты после выхода релиза кнопка скачивания вела на ошибку 404; теперь плашка и кнопки появляются только когда файл реально прикреплён к релизу, и качают его по точной ссылке ([](https://github.com/fiscalagent/AppTochite/commit/4c73bfb15953ec11cf04d6ae9327e10c3304176a))
...
* бэкап в папку на устройстве в приложении наконец работает — теперь вы сами выбираете папку в системном диалоге, и копия сохраняется в неё (на новых Android прямая запись в Документы была запрещена, отсюда ошибка «нет доступа к папке») ([](https://github.com/fiscalagent/AppTochite/commit/791d0ebb9ddee3daf273140cf63dcece8500221b))
* если приложение работает во временном хранилище браузера (данные может стереть Chrome) — теперь предупреждаем об этом и подсказываем установить приложение на экран телефона; при отказе записи в базу показываем ошибку, а не теряем данные молча ([](https://github.com/fiscalagent/AppTochite/commit/8a4379f283930c5811fe01fe16ff5049626f9968))
...
* при «Повторить заточку» в новую заточку переносится микроподвод — его угол и сама отметка ([](https://github.com/fiscalagent/AppTochite/commit/40d1ef37b1489b4bb2322e022b8229c2a7312cf2))
...
* восстановление и объединение бэкапа из облака через CORS-прокси ([](https://github.com/fiscalagent/AppTochite/commit/224287584cae66bb3b6de0a0359c08f703b7a2b7))
